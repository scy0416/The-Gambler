...
const RELIC_UI = preload("res://Card/RelicUI.tscn")
...
func _ready() -> void:
	...
	#유물 시험용 코드
	add_relic(preload("res://Scripts/Resources/allRelics/Sword.tres"))
	add_relic(preload("res://Scripts/Resources/allRelics/BamYangGang.tres"))
	add_relic(preload("res://Scripts/Resources/allRelics/Dumbbell.tres"))
	add_relic(preload("res://Scripts/Resources/allRelics/Barbell.tres"))
	
	
func activate_relics_by_type(type: Relic.Type):
	if type == Relic.Type.EVENT_BASED:
		return
	
	var relic_queue: Array[RelicUI] = _get_all_relic_ui_nodes().filter(
		func(relic_ui: RelicUI):
			return relic_ui.relic.relicType == type
	)
